...
complexWorkers(W1,W2):- workerScore(W1,S1), workerScore(W2,S2), W1 < W2, S is S1+S2, complexThreshold(Max), S > Max.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 1.- Declare SAT variables to be used
% wt(W,T) means "worker W is in team T"
satVariable( wt(W,T) ):- worker(W), team(T).
satVariable( complexTeam(T) ) :- team(T).
...
writeClauses(infinite):- !, complexThreshold(N), writeClauses(N),!. % N = 1000 should be replaced with by right N
writeClauses(MaxComplexTeams):- 
    eachWPairLessThanMaxScore,
    eachWorkerExactlyOneTeam,
    eachTeamMoreThanMinW,
    eachTeamLessThanMaxW,
    fewComplexTeamsThanThreshold(MaxComplexTeams),
    relationWTI,
    true,!.
...
eachWorkerExactlyOneTeam.
eachTeamMoreThanMinW :- team(T), minSize(K), findall(wt(W,T), worker(W), Lits), atLeast(K, Lits), fail.
eachTeamMoreThanMinW.
eachTeamLessThanMaxW :- team(T), maxSize(K), findall(wt(W,T), worker(W), Lits), atMost(K, Lits), fail.
eachTeamLessThanMaxW.
eachWPairLessThanMaxScore :- team(T), incompatibleWorkers(W, W2), writeClause([-wt(W, T), -wt(W2, T)]), fail.
eachWPairLessThanMaxScore.
fewComplexTeamsThanThreshold(MaxComplexTeams) :- findall(complexTeam(T), team(T), Lits), atMost(MaxComplexTeams, Lits), fail.
fewComplexTeamsThanThreshold(_).
relationWTI :- team(T), worker(W1), worker(W2), complexWorkers(W1, W2),
                writeClause([-wt(W1, T), -wt(W2, T), complexTeam(T)]), fail.
relationWTI.
...
costOfThisSolution(M, Cost):- findall(T,member(complexTeam(T),M),L), sort(L,L1), length(L1,Cost), !.
