...
satVariable( t(B, D1, C1, D2, C2) ) :- bus(B), consecutiveDays(D1, D2), trip(C1-C2).

writeClauses:- 
    oneBusEveryDayForEveryCity,
    oneBusOnlyOneCityADay,
    oneBusNotOnTheSameCityForTwoDays,
    noMoreThanMaxDist,
    allTripsOneTime,
    relationTandBDC,
...
oneBusEveryDayForEveryCity :- city(C), day(D), findall(bdc(B, D, C), bus(B), Lits), exactly(1, Lits), fail.
oneBusEveryDayForEveryCity.


oneBusOnlyOneCityADay :- bus(B), day(D), findall(bdc(B, D, C), city(C), Lits), exactly(1, Lits), fail.
oneBusOnlyOneCityADay.


oneBusNotOnTheSameCityForTwoDays :- bus(B), consecutiveDays(D1, D2), city(C), writeClause([-bdc(B, D1, C), -bdc(B, D2, C)]), fail.
oneBusNotOnTheSameCityForTwoDays.


noMoreThanMaxDist :- 	bus(B), consecutiveDays(D1, D2, D3), maxDist(K), trip(C1-C2), trip(C2-C3), dist(C1, C2, S1), dist(C2, C3, S2),
						K < S1+S2,
						writeClause([-bdc(B, D1, C1), -bdc(B, D2, C2), -bdc(B, D3, C3)]), fail.
noMoreThanMaxDist.

allTripsOneTime :- 	trip(C1-C2), findall(t(B, D1, C1, D2, C2), (bus(B), consecutiveDays(D1, D2), notLastDay(D2)), Lits), atLeast(1, Lits), fail. 
allTripsOneTime.

relationTandBDC :- 	bus(B), consecutiveDays(D1, D2), trip(C1-C2), writeClause([- t(B, D1, C1, D2, C2), bdc(B, D1, C1)]), writeClause([- t(B, D1, C1, D2, C2), bdc(B, D2, C2)]), fail.
relationTandBDC.
