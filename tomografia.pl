...
split(0, Residu, [], Residu) :- !.
split(N, [E|L], [E|Inici], Residu) :- N2 is N-1, split(N2, L, Inici, Residu).

matrixByRows([], _, []) :- !.
matrixByRows(L, NumCols, [X | MatrixByRows]) :- split(NumCols, L, X, Residu),
												matrixByRows(Residu, NumCols, MatrixByRows).

sumaCorrecta(MatrixBy, Suma) :- sum(MatrixBy, #=, Suma).

declareConstraints(MatrixByRows, MatrixByCols, RowSums, ColSums) :- 
			maplist(sumaCorrecta, MatrixByRows, RowSums),
			maplist(sumaCorrecta, MatrixByCols, ColSums).
