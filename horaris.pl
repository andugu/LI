...
% professor that has at least one course
satVariable(teaching_prof(P)):- professor(P).
% course C taught on day D
...
writeClauses(MaxNumProf) :-
    oneCourseNDayHour,
    oneCouseOneProfessor,
    oneCourseOneRoom,
    maxTeachingProfs(MaxNumProf),
    teachingPr,
    oneClassDay,
    courseDay,
    noSameYearRepetition,
    respectLawsOfPhysicsProfessor,
    respectLawsOfPhysicsRoom,
    notAllowedProf,
    notAllowedRoom,
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

notAllowedProf :- course(C), courseProfessors(C, LP), professor(P), \+ member(P, LP), writeClause([-cp(C, P)]), fail.
notAllowedProf.
notAllowedRoom :- course(C), courseRooms(C, LR), room(R), \+ member(R, LR), writeClause([-cr(C, R)]), fail.
notAllowedRoom.

% H clases a la semana
oneCourseNDayHour :- course(C), courseHours(C, H), findall(cd(C,D), day(D), Lits), exactly(H, Lits), fail.
oneCourseNDayHour.

% Una clase por dia
oneClassDay :- course(C), day(D), findall(cdh(C, D, H), hour(H), Lits), atMost(1, Lits), fail.
oneClassDay.

% Relacion entre cd i cdh
courseDay :- course(C), day(D), findall(cdh(C, D, H), hour(H), Lits), expressOr(cd(C, D), Lits), fail.
courseDay.

% Un professor por curso
oneCouseOneProfessor :- course(C), findall(cp(C, P), professor(P), Lits), exactly(1, Lits), fail.
oneCouseOneProfessor.

% Una aula por curso
oneCourseOneRoom :- course(C), findall(cr(C, R), room(R), Lits), exactly(1, Lits), fail.
oneCourseOneRoom.

% Menor numero de profesores posibles
maxTeachingProfs(MaxNumProf) :- findall(teaching_prof(P), professor(P), Lits), atMost(MaxNumProf, Lits), fail.
maxTeachingProfs(_).

% Relacion entre cp i teaching_prof
teachingPr :- professor(P), findall(cp(C, P), course(C), Lits), expressOr(teaching_prof(P), Lits), fail.
teachingPr.

% No dos clases simultaneas por un mismo profesor
respectLawsOfPhysicsProfessor :- course(C1), course(C2), C1 < C2, day(D), hour(H), professor(P), writeClause([-cdh(C1,D,H), -cdh(C2,D,H), -cp(C1,P), -cp(C2,P)]), fail.
respectLawsOfPhysicsProfessor.

% No dos clases simultaneas en una misma clase
respectLawsOfPhysicsRoom :- course(C1), course(C2), C1 < C2, day(D), hour(H), room(R), writeClause([-cdh(C1,D,H), -cdh(C2,D,H), -cr(C1,R), -cr(C2,R)]), fail.
respectLawsOfPhysicsRoom.

% No solapamiento entre dos cursos del mismo año
noSameYearRepetition :- courseYear(C1, Y), courseYear(C2, Y), C1 < C2, day(D), hour(H), writeClause([-cdh(C1, D, H), -cdh(C2, D, H)]), fail.
noSameYearRepetition.
...
costOfThisSolution(M, Cost):- findall(teaching_prof(P), member(cp(_, P), M), L), sort(L,L1), length(L1, Cost), !.
