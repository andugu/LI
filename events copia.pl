...
writeClauses:- 
	oneEventOneDay,
	oneModeratorOneEvent,
	noMoreEventsPerDay,
	%noMoreEventsPerModerator,
	relationMD,
    true, !.                    % this way you can comment out ANY previous line of writeClauses

writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

oneEventOneDay :- event(E), findall(ed(E, D), day(D), Lits), exactly(1, Lits), fail.
oneEventOneDay.

oneModeratorOneEvent :- event(E), findall(em(E, M), moderator(M), Lits), exactly(1, Lits), fail.
oneModeratorOneEvent.

noMoreEventsPerDay :- day(D), maxEventsPerDay(K), findall(ed(E, D), event(E), Lits), atMost(K, Lits), fail.
noMoreEventsPerDay.

noMoreEventsPerModerator :- moderator(M), maxDaysPerModerator(K), findall(md(M, D), day(D), Lits), atMost(K, Lits), fail.
noMoreEventsPerModerator.

relationMD :- eventModerators(E, LM), member(M, LM), eventDays(E, LD), member(D, LD), writeClause([-em(E, M), -ed(E, D), md(M, D)]), fail.
relationMD.
